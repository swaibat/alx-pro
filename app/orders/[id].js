  ...
  return (
    <>
      ...
      <Appbar.Header style={{ paddingRight: 15, backgroundColor: '#111b2d' }}>
        <Appbar.BackAction color={theme['color-basic-100']} onPress={() => router.push('orders')} />
        <Appbar.Content color={theme['color-basic-100']} title={<Text style={{ color: theme['color-basic-100'], fontSize: 18 }}>Order Details</Text>} />
      </Appbar.Header>
      <Divider />

      <ScrollView style={styles.container}>
        {isLoading && <View style={styles.errorContainer}>
          <ActivityIndicator size="large" color={theme['color-primary-default']} style={styles.loader} /></View>}
        {isError && <View style={styles.errorContainer}>
          <Text category="h6" style={styles.errorText}>Error fetching order details.</Text>
        </View>}
        {order && <>
          <View style={styles.card}>
            ...
            <Text category="s1" style={{ marginBottom: 5 }} appearance='hint'>{order.status}</Text>
          </View>
            ...
            {order.items.map((item, index) => (
              <View key={item.productId} style={styles.itemCard}>
                <View style={styles.itemContainer}>
                  <Image source={item.file ? { uri: item.file } : require('@/assets/placeholder.png')} style={[styles.itemImage, { backgroundColor: theme['color-basic-300'] }]} />
                  <View style={styles.itemDetails}>
                    <Text category="s1" numberOfLines={1}>{item.title}</Text>
                    <Text category="p2" appearance="hint" numberOfLines={1} style={{ marginBottom: 4 }}>{item.description}</Text>
                    <Text category="s2" appearance="hint" style={{ marginBottom: 4 }}>Qty: {item.quantity}</Text>
